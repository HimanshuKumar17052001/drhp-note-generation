...
class UploadToBlob:
    def __init__(self):
        ...
        logger.info(f"Initializing S3 client for bucket: {self.bucket_name} in region: {aws_region}")
    ...
    def upload_to_blob(self, file_obj, file_name: str) -> Tuple[bool, str]:
        try:
            ...
            try:
                ...
                logger.error(f"[UPLOAD_ERROR] Error during S3 upload: {str(upload_error)}")
            ...
            try:
                ...
                logger.error(f"[UPLOAD_ERROR] Error generating pre-signed URL: {str(url_error)}")
                logger.error(f"[UPLOAD_TRACEBACK] {traceback.format_exc()}")
                return False, f"Pre-signed URL generation error: {str(url_error)}"

            logger.info(f"[UPLOAD_COMPLETE] Successfully completed upload process for {file_name}")
            return True, url
    ...
    def delete_file(self, file_url: str) -> Tuple[bool, str]:
        try:
            logger.info(f"[DELETE_START] Attempting to delete file with URL: {file_url}")
            ...
            if 's3.amazonaws.com/' in file_url:
                ...
                logger.debug(f"[DELETE_DEBUG] Parsed standard S3 URL to key: {file_path}")
            elif '.s3.' in file_url:
                ...
                logger.debug(f"[DELETE_DEBUG] Parsed virtual hosted style URL to key: {file_path}")
            else:
            ...
            # Delete the object
            logger.debug(f"[DELETE_DEBUG] Deleting object from bucket: {self.bucket_name}, key: {file_path}")
            try:
                ...
                logger.error(f"[DELETE_ERROR] Error during delete operation: {str(delete_error)}")
    ...
    def check_new_files(self, time_to_check_in_mins=1) -> List[dict]:
        try:
            logger.info(f"[CHECK_NEW_FILES_START] Checking for new files modified in the last {time_to_check_in_mins} minutes")
            ...
            logger.debug(f"[CHECK_NEW_FILES_DEBUG] Looking for files modified after: {time_to_check}")
            ...
            try:
                ...
                for page in paginator.paginate(Bucket=self.bucket_name):
                    if 'Contents' not in page:
                        logger.debug(f"[CHECK_NEW_FILES_DEBUG] No contents in this page of results")
                        continue
                    
                    logger.debug(f"[CHECK_NEW_FILES_DEBUG] Found {len(page['Contents'])} objects in this page")
                    for obj in page['Contents']:
                        try:
                            last_modified_utc = obj['LastModified'].astimezone(pytz.UTC)
                            
                            # Check if file matches criteria
                            is_valid_extension = obj['Key'].endswith(('.mp3', '.mp4', '.wav', '.m4a', '.aac'))
                            is_recent = last_modified_utc >= time_to_check
                            
                            logger.debug(f"[CHECK_NEW_FILES_DEBUG] File: {obj['Key']}, Last Modified: {last_modified_utc}")
                            logger.debug(f"[CHECK_NEW_FILES_DEBUG] Valid extension? {is_valid_extension}, Recent? {is_recent}")
                            ...
                            if is_recent and is_valid_extension:
                                ...
                                logger.info(f"[CHECK_NEW_FILES_SUCCESS] Added file to results: {obj['Key']}")
                            else:
                                continue
                        except Exception as obj_error:
                            logger.error(f"[CHECK_NEW_FILES_ERROR] Error processing object: {str(obj_error)}")
                            continue

                logger.info(f"[CHECK_NEW_FILES_COMPLETE] Found {len(new_files)} new files")
                return new_files

            except Exception as list_error:
                logger.error(f"[CHECK_NEW_FILES_ERROR] Error listing objects: {str(list_error)}")
                logger.error(f"[CHECK_NEW_FILES_TRACEBACK] {traceback.format_exc()}")
                return []

        except Exception as e:
            logger.error(f"[CHECK_NEW_FILES_ERROR] Unexpected error in check_new_files: {str(e)}")
            logger.error(f"[CHECK_NEW_FILES_TRACEBACK] {traceback.format_exc()}")
            return []

    def download_file(self, file_name: str, download_path: str):
        try:
            logger.info(f"[DOWNLOAD_START] Starting download for file: {file_name} to path: {download_path}")
            ...
            try:
                ...
                logger.info(f"[DOWNLOAD_SUCCESS] File downloaded successfully to {download_path}")
            except Exception as download_error:
                logger.error(f"[DOWNLOAD_ERROR] Error during file download: {str(download_error)}")
            ...
            logger.error(f"[DOWNLOAD_ERROR] Unexpected error in download_file: {str(e)}")
            logger.error(f"[DOWNLOAD_TRACEBACK] {traceback.format_exc()}")
            return False, str(e)
    ...
    def check_file_exists(self, file_path: str) -> bool:
        try:
            ...
            logger.error(f"[CHECK_FILE_ERROR] Unexpected error in check_file_exists: {str(e)}")
